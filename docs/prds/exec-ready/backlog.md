...
## bugfix-redfirst (5.19.0) — Future opportunities captured 2026-06-13

**BRF-1: Live e2e routing scenario for AC-2 producer paths.** The bugfix-redfirst piece validated the `**Phase type:** bug-fix|regression` → `tdd: true` front-matter write via structural fixtures only (plan text checks, not live routing). The spec's Testing Strategy called for a live e2e scenario exercising both producer paths (plan skill and small-change Step 9) against a `tdd: false` repo config. Candidate: extend the `pipeline-e2e` fixture project with a `tdd: false` repo-config scenario where a regression-classified phase yields `tdd: true` in the emitted plan front-matter, and the small-change Step 9 path writes `tdd: true` inline. Exercises the actual routing prose rather than only asserting a pre-authored fixture carries the right markers. **Deps:** pipeline-e2e (merged), bugfix-redfirst (merged).

**BRF-2: Unify execute Step 6c discovery menu with 5-disposition triage-contract vocabulary.** Execute's Step 6c agent-discovery aggregation still uses a bespoke 3-option (a)/(f)/(d) vocabulary instead of the shared 5-disposition triage-contract. The two layers now carry divergent vocabularies: the menu says 3 options, the contract says 5. A bug-classified finding has no path to `note-on-scheduled` or `explicit-defer-with-rationale` through the current menu. Candidate: replace the (a)/(f)/(d) bespoke menu in `execute/SKILL.md` Step 6c with the contract's canonical 5-disposition map; cite the contract rather than restating dispositions inline. Scope: Step 6c only; no behavior change for the three current dispositions, only two missing dispositions added. **Deps:** discovery-triage (done), bugfix-redfirst (merged — triage-contract now has the consumer-honor clause).

**BRF-3: Form C schema `bug_classified` field for campaign-source findings.** When `outcome-campaign` ships, its fix-routing handoff to `spec-flow:triage` will need the NN-P-006 stamp applied automatically. Today's triage-contract `## Red-first obligation` consumer-honor clause (shipped by bugfix-redfirst Phase 6) relies on the operator recognizing a campaign bug-finding as needing red-first. The Form C batch schema needs a `bug_classified: true` field (or equivalent) so triage can apply the obligation automatically. Candidate: during `outcome-campaign` spec brainstorm, add a resolved open question defining the field; extend the triage-contract section with a campaign-source annotation; add a clean-control fixture (a campaign finding NOT classified as a bug that does NOT inherit the stamp). **Deps:** bugfix-redfirst (merged), outcome-campaign (open).
