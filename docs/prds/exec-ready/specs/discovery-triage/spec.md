...
- **Execute regression (FR-T10/FR-T11):** the execute surgery is covered by static-citation assertions (see static.sh FR-019/FR-023 block) and the live-run procedure; `pipeline-e2e` (merged) is the execute regression net for the unified path (admission → shared-contract classification → scope→amend→execute) so the rewire cannot regress execute's externally-observable behavior.
