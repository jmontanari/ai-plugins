...
- FR-009-N: Opus pre-flight for thinking stages (inverse of execute). The `spec`, `prd`, `plan`, and `charter` skills run a pre-flight model check at skill start — the exact mirror of execute's existing Sonnet check (`skills/execute/SKILL.md` Step 0). It determines the active main-loop model by the platform-appropriate method; if the active model name does NOT contain `opus` (case-insensitive), it surfaces a one-line recommendation to switch to an Opus-class model (spec/plan/charter authoring is thinking work per NN-P-005) and prompts the operator. The prompt is **advisory, not a hard block** — the operator may proceed on the current model — mirroring execute's non-blocking Sonnet warning. On Opus, the check is a silent no-op (or a one-line confirmation, consistent with execute's per-stage model-policy report). This reuses the execute pre-flight pattern rather than inventing a new one. (small-change is excluded — it is a lightweight single-session flow that defaults to lite depth; an Opus pre-flight there would be disproportionate.)
...
AC-24: Given the `spec`, `prd`, `plan`, or `charter` skill is invoked, When it starts, Then it runs a pre-flight model check (the Opus mirror of execute's Sonnet check): it determines the active model and, if the active model name does not contain `opus` (case-insensitive), surfaces a one-line recommendation to switch to an Opus-class model and prompts the operator; the prompt is advisory (operator may proceed), not a hard block; on Opus it is a silent no-op or one-line confirmation.
  Independent Test: `grep -n -i 'opus' plugins/spec-flow/skills/spec/SKILL.md plugins/spec-flow/skills/plan/SKILL.md plugins/spec-flow/skills/charter/SKILL.md plugins/spec-flow/skills/prd/SKILL.md` each return a pre-flight model-check block that warns when the active model is not Opus-class; the block does not hard-block (no refuse/exit on non-Opus).
...
1. Classify the answer against deliberation.md:
     in §Viability Analysis evaluated paths? → accept (covered), next question
     in §Answered by Investigation?          → accept (covered), next question
     in NEITHER → it is a NEW ASSERTION → step 2
...
`skills/{spec,prd,plan,charter}/SKILL.md` add a skill-start model check that is the mirror of execute's Step 0 Sonnet check (`skills/execute/SKILL.md` lines ~15–26): determine the active model; if its name does not contain `opus` (case-insensitive), emit a one-line recommendation to switch to Opus (thinking work per NN-P-005) and prompt the operator. Advisory and non-blocking — the operator may proceed — exactly like execute's Sonnet warning is non-blocking. On Opus, silent no-op or a one-line confirmation. small-change is excluded (lite, lightweight single-session flow).
