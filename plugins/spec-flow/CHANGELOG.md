...
- **`agents/deliberation-viability.md`:** Phase B (dispatched in parallel, one per decision-unit cluster) — enumerates ALL viable paths for its cluster including reuse/extend-existing paths, assigning VIABLE/NON-VIABLE with a concrete blocker for each NON-VIABLE. No sub-agents; `model: opus`.
- **`agents/deliberation-synthesis.md`:** Phase C (runs only when ≥2 clusters) — checks cross-cluster path composition, documents conflicts, narrows to composable paths, and produces an integrated recommendation. No sub-agents; `model: opus`.
- **`agents/deliberation-lens.md`:** Phase D single-lens adversarial agent, instantiated once per lens via a `{lens}` parameter slot (the five lenses: architecture-integrity, scope/simplicity, user-intent, backward-compat, risk). Returns HOLDS or CONTESTED. Single-model multi-lens board (not multi-model). No sub-agents; `model: opus`.
- **`agents/deliberation-convergence.md`:** Phase E — folds the Phase D adversarial verdicts, finalizes the recommendation, assigns stable VOQ-N IDs to surviving unresolved questions, records the resolved depth, and WRITES the 7-core-section `deliberation.md`. No sub-agents; `model: opus`.
- **`agents/deliberation-validate.md`:** Tier 2 answer-validation agent — auto-fired during brainstorm when an operator's free-form answer introduces an assertion outside the evaluated path-set; validates that single assertion and returns CONFIRM / FLAG-HARD (binding-NN violation, no override) / FLAG-SOFT (override with rationale), appending a Validation Round. No sub-agents; `model: opus`.
- **`reference/deliberation-artifact.md`:** canonical contract for the `deliberation.md` artifact: the 7 core H2 sections + optional 8th `## Validation Rounds`, the VOQ-N ID contract, the four markers (CONSUMED/ABSENT/UNAVAILABLE/SKIPPED), and the ≤2K/STATUS return contract. Cited by all six deliberation agents and the calling skills.
- **`reference/deliberation-depth.md`:** defines the three depth profiles — full / lite / off (not tiers) — the per-skill defaults (full for spec/prd/charter, lite for small-change), the lite lens subset (scope/simplicity + risk), the resolution order (operator override → config → per-skill default), and the ≤1-cluster Phase-C no-op rule.
- **`.spec-flow.yaml` `deliberation.depth` / `deliberation.lenses` config keys:** optional operator overrides for depth profile and the lite lens subset; absent keys resolve to per-skill defaults (NN-C-003 backward-compatible).

### Changed
- **`skills/{spec,prd,small-change,charter}/SKILL.md` — deliberation protocol wiring:** each skill dispatches Phase A→B (barrier)→C (skipped at ≤1 cluster)→D (barrier)→E before its first question; questions are then restricted to VOQ-N-cited validated open questions. spec/prd/charter run full depth; small-change runs lite. Also adds the shared Tier-2 answer-validation loop.
- **`skills/plan/SKILL.md` — Phase-1 consumption of `deliberation.md`:** reads §Recommendation + §Viability Analysis and emits `[DELIBERATION-CONSUMED]` (present) or `[DELIBERATION-ABSENT]` (absent), feeding the recommendation to the plan agent as the approach anchor.
- **Opus pre-flight added to spec, prd, plan, and charter skills (small-change excluded per FR-009-N):** each recommends an Opus-class model at skill start (3 choices; Override always proceeds; silent when already Opus) — the inverse of execute's Sonnet check.
