...
- **`[integration]` test carve-out and completing-phase rules.** A registered `[integration]` test whose `completes_in_phase` is **after** the current phase is expected-red — do NOT treat it as a missing Red test, do NOT attempt to green it, and do NOT raise a spurious BLOCK. At its completing phase, greening the `[Integration-Test]` block is **wiring glue** (the in-boundary components already exist from prior phases), so the minimal-code law applies to the wiring only; do NOT raise a spurious BLOCK on the grounds that "no production code is needed." Also write the **contract tests** for each doubled true external so the doubles remain faithful to the real interface, and respect the M3 window — only edit the registered `[integration]` test in its `completes_in_phase` phase (the M3 fixture/helper closure prevents edits outside that window).
