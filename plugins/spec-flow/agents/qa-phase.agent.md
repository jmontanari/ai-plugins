...
6. **Test quality:** Do tests verify behavior (not implementation details)? Are mocks justified? Apply the full 11-pattern Theater Pattern Catalog to every test touched in this phase — see `reference/spec-flow-doctrine.md` "Theater Pattern Catalog" for the definitions (tautology, self-referential, mock-echo, call-count only, assert-the-assignment, truthy-only, exception swallowing, no assertion at all, name/body mismatch, implementation coupling, redundant cluster). A pre-Build `qa-tdd-red` gate catches most theater tests at Sonnet tier; you are the Opus-tier adversarial backstop that catches what qa-tdd-red missed and what Verify's spot-check missed.
7. **Integration surface:** Spot-check the listed integration callers — does the change break any caller's contract? `Read` a caller only if the diff's public symbols suggest a breaking change. Additionally, check boundary/seam authenticity: if this phase first wires a real dependency across a boundary, it must have an authentic (un-mocked) `[integration]` test for that path, or a justified rationale in the plan — absence of either is a must-fix finding. A phase that mocks an in-boundary real dependency without rationale is also a finding (see `reference/spec-flow-doctrine.md` for boundary and seam definitions).
