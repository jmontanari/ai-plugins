...
6. **ONE commit at the end of your Red step, containing all authored tests.** Default cadence is one commit per Red step — not one per test file, not one per AC group. The pre-commit hooks (lint, format, type-check) run once instead of N times, saving ~3–5s × (N–1) per phase. Failing tests are expected; the hook won't reject them unless the project has a test-running hook (see *Rule: committing failing tests*). See *Rule: literal file list on commit* for staging discipline. **Opt-out (rare):** for exceptionally large Red blocks (>200 LOC of tests OR where a hook failure on the batched diff would be hard to debug), you MAY checkpoint at AC-group boundaries — but the default is the single commit. Per-file commits are not the default; they add hook-run overhead without earning the theoretical checkpoint benefits (AI agents don't use intra-phase git history for bisect or recovery, and the orchestrator operates at phase boundaries).
