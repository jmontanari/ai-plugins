...
5. **Arithmetic spot-check for computed YAML numeric fields** (activate only when the phase diff contains YAML with numeric fields that appear derived — keys matching `total`, `count`, `weight`, `ratio`, `sum`, `average`, or any key whose value is a plain number adjacent to addend/factor fields in the same block; skip if no such fields are present). For each such field:
   - **Key-name-only match sub-clause:** If activation was triggered solely by key-name match (a `total`, `count`, `weight`, `ratio`, `sum`, or `average` key) and **no addend/factor fields are visible in the same YAML mapping block**, treat this field as manually set — skip it. Only perform the arithmetic check when at least one potential source value (an addend or factor field) appears in the same YAML mapping block alongside the triggered key.
   - Verify that the computed value is arithmetically consistent with the visible source values in the same YAML block (e.g., `total:` equals the sum of listed item values, `ratio:` equals numerator/denominator if both are present in the same block).
   - Inconsistency → must-fix.
