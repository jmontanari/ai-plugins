...
| Pointer | always | For `covered`: a concrete `file:line` (TDD mode) or a concrete assertion reference inside the `[Verify]` command (Implement mode). Integration tests use the form `tests/x.py:N [integration]` (path + line + tag), which is a valid concrete pointer — path coverage is tracked orthogonally to AC coverage. For `NOT COVERED — deferred to <pointer>`: the phase or spec amendment where the AC will be picked up (e.g. `Phase N+1 per plan.md:L120`). For bare `NOT COVERED`: `—` (em dash). |
...
4. **Vague `covered` pointer.** A `covered` row whose Pointer column lacks a concrete `file:line` (TDD mode) or a concrete assertion reference inside the `[Verify]` command (Implement mode). **Exception:** a pointer of the form `tests/x.py:N [integration]` (concrete path + line number + `[integration]` tag) is accepted as a valid concrete pointer — it identifies an integration test by file and line, with path coverage tracked orthogonally to AC coverage. Examples that fail validation: `see test file`, `covered by integration tests`, `tests/foo.py` (no line), `the build runs`. The pointer must be unambiguously verifiable. A bare category reference such as "covered by integration tests" (no file:line) fails this rule even if the word "integration" appears.
