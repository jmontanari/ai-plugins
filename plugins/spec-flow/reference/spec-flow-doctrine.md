...
**Deferred commit for Phase Groups (v5.0.0).** Under `deferred_commit: auto`, a Phase Group of parallel TDD sub-phases runs **git-free**: neither Red nor Build stages or commits during the group (the deferred-group flag in each agent's prompt suppresses their normal `git add`/`git commit`). This removes the shared-index race that corrupts parallel sub-phases each calling `git add`. The whole group then collapses into ONE **barrier work-commit** covering the union of every sub-phase's (Red manifest paths ∪ Build production paths). Because the git-free files are untracked, the barrier uses an **add-then-commit pathspec** — `git add -- <union>` then `git commit -- <union>` (a bare `git commit -- <union>` fails on the untracked files). A **separate** plan.md progress commit follows the work-commit, so a deferred group lands exactly 2 commits regardless of sub-phase count. The anti-cheat re-hashes the Red **test files only** against the **group journal's** `red_manifest_hashes` **in the working tree** at the barrier (NOT against test files in HEAD — the tests aren't in any commit until the barrier work-commit itself lands). Production files in scope are **trusted by association** (NOT re-hashed — ADR-2), the same trust model the flat-phase gate uses. It detects test-file tampering during Build; it does NOT detect production-file drift across a resume (a known limitation — deeper integrity anchoring is deferred to the Tier-2 / `journal_tier` future). It is the working-tree analogue of the flat-phase content-hash gate, not a strictly stronger check. Flat phases and any run under `deferred_commit: off` keep the per-phase unified-commit model above unchanged. See `reference/deferred-commit-journal.md` for the journal schema, resume algorithm, file-scoped recovery recipe, and the barrier-commit recipe.
