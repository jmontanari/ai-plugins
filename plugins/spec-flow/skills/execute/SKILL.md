...
Read the `amendment_budget` key from `.spec-flow.yaml` in the SAME pass (valid values: a positive integer; default `5` when absent/unset — NN-C-003; malformed value, `off`, `unlimited`, or any non-positive integer → one-line warning + store `5` as the resolved value, never the invalid parsed value). Store as `amendment_budget` in orchestrator state; the `#### Amendment budget tracking` check uses it as the hard-cap threshold. This does NOT govern the 1-spec-amendment sub-cap (a fixed constant of 1), the oracle 2-attempt build budget, or the spike-phase soft-checkpoint.
...
7. **Post-commit integrity and reconciliation gates (Mode: TDD + Implement, v3.1.1+).** After the implementer's commit lands (HEAD now points to it), run cheap checks before accepting the phase. Gate (a) is TDD-only (uses Red's manifest); gate (b) is HARD FAIL on BOTH modes — strays or missings reject the phase. The Implement-track extension was added in v3.1.1 per pi-009-hardening's Phase Group A contamination event, where A.2 silently swept in A.4's staged files because the gate was previously gated `Mode: TDD only`.
   ...
   - **(a) Content-hash integrity (Mode: TDD only).** For every path in `phase_N_red_stage_manifest`, re-hash the file AS COMMITTED in HEAD and compare against the manifest:
     ...
     ```
     Any mismatch for a path that is **not** in `exempt_authored` means the implementer modified one of Red's tests — the anti-cheat safeguard replacing pre-v2.7.0's `git diff tests/` check. (A path in BOTH the manifest and `exempt_authored` is a smuggling violation — it is also rejected, per the T-1 precedence rule.) Collect all violating paths and reject the phase: **name each violating path explicitly** in the re-dispatch to the implementer (same surfacing pattern as SKIPPED-IDs at Step 3 items 5b/5c). Retry within the 2-attempt budget (the retry must recreate the commit without touching Red's tests or `exempt_authored` overlap paths). Escalate on second failure. Do NOT add any warn-and-proceed or "continue anyway" branch — integrity failures are hard stops. **Multi-commit (rare opt-out).** When the implementer exercises the rare opt-out (multiple commits for phases >200 LOC delta), gate (a) runs after EACH intermediate commit — not only after the final commit. An intermediate commit that fails gate (a) is rejected immediately; the implementer must amend or fixup that specific commit before the phase continues. This prevents tamper-and-restore patterns that would pass a final-only check.
     ...
     **`exempt_authored` set (Implement-track declared test authorship).** When the current phase's plan section includes an `**Authored-tests:**` field, parse the listed literal paths into `exempt_authored = { paths }`. If the field is absent or empty, `exempt_authored = ∅`. **Path normalization:** before any intersection check, normalize every path in `exempt_authored` AND every path in `phase_N_red_stage_manifest` to their canonical forms — resolve relative prefixes (`./`), collapse `..` components, and strip trailing slashes — so that equivalent paths expressed differently compare equal (CWE-706 guard). Evaluation order: M3 registry window → Red-manifest immutability → `exempt_authored` (lower precedence). A path in BOTH `phase_N_red_stage_manifest` and `exempt_authored` is a **HARD REJECT (smuggling guard)** — the exemption is ignored; the conflict is surfaced as an integrity failure. **This smuggling guard applies to ALL entries in the Red manifest, including Phase-2-enriched entries (conftest.py and directly-imported fixture/helper files added via tdd-red Rule 10 enrichment) — not only primary test file entries.** This is plan-derived (the `**Authored-tests:**` field is qa-plan-reviewed), gate-tightening, and never a merge path (NN-P-002). On Implement-track phases there is no Red manifest, so `exempt_authored` is the sole mechanism that admits the authored test in gate (b) reconciliation.
   ...
   - **(b) Unified commit reconciliation (Mode: TDD AND Mode: Implement).** The commit's file list must equal the **expected file set**:
     - **Mode: TDD:** `expected = Red's manifest paths ∪ Build's \`## Files Created/Modified\` paths ∪ exempt_authored`.
     - **Mode: Implement:** `expected = Build's \`## Files Created/Modified\` paths ∪ exempt_authored` (no Red manifest; declared authored tests are whitelisted via `exempt_authored` so they do not trigger a stray-file flag).
...
4. Dispatch:
   ...
   ```
5. **Test integrity (Mode: TDD only; non-TDD mode: no-op).** This is an **unconditional phase-exit check** — any mismatch is a **blocking finding attributed to the phase** (REJECT + revert the offending commit + re-dispatch the responsible agent with the violating paths named), never silently absorbed or treated as a no-op because an earlier gate already passed. The check covers the full Red manifest set, including Phase-2-enriched fixture and conftest paths. In non-TDD mode (`tdd: false`), there is no Red manifest, so this check is a no-op. The primary anti-tampering gate runs at Step 3.7a (content-hash check of Red's staged test manifest against Red's test files in HEAD). The Step 4 exit re-hash is the **second, phase-exit opportunity** to catch drift that landed after Step 3.7a — in particular, drift introduced during a Refactor commit (Step 5): re-run the content-hash check against HEAD after Refactor lands (Refactor is phase-scoped and must not touch test files the Red agent authored; re-hashing catches drift). For registered `[integration]` paths, re-hash against `skeleton_sha256` (for phases before `completes_in_phase`) or `completed_sha256` (at/after `completes_in_phase`) — AND re-hash their declared fixture/helper closure — so that Refactor cannot swap a real in-boundary dependency for a test double in a helper file after Refactor (the integration-preservation backstop for Phase 7's refactor.md rule). If any hash drifts: REJECT, revert the offending commit, and re-dispatch the responsible agent with the violating paths surfaced.
...
4. **Repeated immutability rejection.** When gate (a) or Step G9b rejects the same phase a 2nd time within the 2-attempt budget — the implementer cannot complete the phase without touching Red's tests — the orchestrator captures a discovery with `default_triage: "amend"`, `source_agent: "orchestrator"` (the orchestrator, not an agent, detects the rejection count), `ac_id: "—"`, and `row_text` = the named violating paths + "repeated immutability rejection — plan/test-data likely under-scoped." This routes the repeated rejection to plan-incompleteness review. The routing NEVER auto-exempts the touched test: mutability is reachable only via a plan amendment that adds the path to `**Authored-tests:**`, re-reviewed by qa-plan under criterion 32 (no collision with the Red manifest) and re-gated by per-phase QA — never via a silent auto-exemption at rejection time.
...
When both step 1 (total budget) and step 2 (spec sub-cap) would fire simultaneously for the same dispatch — i.e. `piece_amendment_count >= amendment_budget` AND the choice is `amend-spec` AND `piece_spec_amendment_count >= 1` — surface a single merged hard-cap escalation: `Hit spec sub-cap AND total amendment budget — this piece may be under-scoped. Choose: (raise) raise amendment_budget and resume / (f) fork / (d) defer / (b) block`. Do not fire two sequential prompts.

1. If `piece_amendment_count >= amendment_budget`, HALT with the diagnostic hard-cap escalation (below) — reaching the cap is a hard stop, not a soft checkpoint. The discovery itself does NOT consume a budget slot unless the operator raises the budget and resumes — only successful dispatches increment counters.
2. If the choice is `amend-spec` AND `piece_spec_amendment_count >= 1`, HALT with the **Amendment hard-cap escalation** (below) — the spec-amend sub-cap is a fixed constant of 1, not configurable via `amendment_budget`; it reuses the same escalation flow. **Exception:** when this halt fires without step 1 also firing (i.e., `piece_amendment_count < amendment_budget`), the `(raise)` option in the shared escalation prompt does NOT apply — raising `amendment_budget` cannot unblock a spec sub-cap halt (the sub-cap is a fixed constant). In spec-sub-cap-only halts, omit `(raise)` from the presented prompt and replace with the note: `spec sub-cap reached (fixed constant of 1 — not configurable); only fork, defer, or block are available.` The discovery itself does NOT consume a budget slot unless a new amend dispatch follows — only successful dispatches increment counters.
...
- ADDITIONALLY for a successful spec amend: `piece_spec_amendment_count++`. Spec amendments increment BOTH counters — they consume one slot of the `amendment_budget` total budget AND the 1-spec-max sub-budget.
...
**Amendment hard-cap escalation.** When `piece_amendment_count >= amendment_budget` (or `piece_spec_amendment_count >= 1` for a spec amendment) would block an amend dispatch, the orchestrator HALTS and emits a diagnostic escalation. The count is at threshold — the piece may be under-scoped. Before presenting options, summarize the amendment history: emit the amendment count from `git log --oneline $piece_start_sha..HEAD --grep '^chore(plan): amend' --grep '^chore(spec): amend' | wc -l` and list `.discovery-log.md` amend rows when the file is present. Then present:

```
Hit <N>/<amendment_budget> amendments — this piece may be under-scoped. Is a larger spec/plan change needed?
Amendment history: <summary from git log + .discovery-log.md amend rows>

Choose:
  (raise) raise amendment_budget in .spec-flow.yaml and resume
...
- **On `raise` (raise and resume):** the operator updates `amendment_budget` in `.spec-flow.yaml` to a value above `piece_amendment_count`, then re-runs execute. This is an attended operator decision — auto-mode under `--auto` cannot raise the budget and falls back to `(d)` defer for the current finding when no operator is present (unattended `--auto` run).
...
The count never resets within a piece. The hard-cap fires at threshold and halts — the operator's `(raise)` choice is a permanent increase to `amendment_budget` (not a per-event unlock). The spec sub-cap of 1 is a fixed constant and uses this same escalation flow. See `plugins/spec-flow/reference/spike-agent.md` `## Amendment budget (spike-phase soft-checkpoint + execute hard-cap)` for the canonical definition.
...
1. **Re-hash each sub-phase's Red tests in the working tree against the journal `red_manifest_hashes`.** For every sub-phase entry in the journal `sub_phases`, re-hash each Red test file (the keys of that sub-phase's `red_manifest_hashes`) **as it sits in the working tree** (not as committed in HEAD — there is no per-sub-phase HEAD commit) and compare against the orchestrator-written blob SHA recorded at `red-done` (FR-2):
   ...
   **Per-sub-phase `exempt_authored` attribution rule (EG-2 fix).** Each sub-phase's `exempt_authored` is derived from THAT sub-phase's own `**Authored-tests:**` field in the plan (mirroring the flat-phase derivation at Step 3 item 7(a)). If the field is absent, `exempt_authored` is the empty set — no drift is exempt. An exemption declared by sub-phase A confers NO exemption when the same path appears in sub-phase B's `red_manifest_hashes` and drifts — the exemption is attributed strictly to the declaring sub-phase. A group-wide exemption union is WRONG and would allow cross-sub-phase tampering to pass undetected (EG-2).
   ...
   Any mismatch for a path that is **not** in the sub-phase's `exempt_authored` set means a Build agent modified one of Red's tests to make it pass. (A path in BOTH `red_manifest_hashes` and the sub-phase's `exempt_authored` is a smuggling violation — rejected per the T-1 precedence rule.) This gate re-hashes the Red **test files only** — production files in each sub-phase's scope are **trusted by association** (NOT re-hashed — ADR-5), the same trust model the flat-phase gate uses for production code. It detects test-file tampering during Build; it does NOT detect production-file drift across a resume (a known limitation — deeper integrity anchoring is deferred to the Tier-2 / `journal_tier` future in `reference/deferred-commit-journal.md`). It is the working-tree analogue of the flat-phase content-hash gate (evaluated against the working tree instead of HEAD because there is no per-sub-phase HEAD commit), not a strictly stronger check. Collect all violating paths and **name each explicitly** in the offending sub-phase's re-dispatch (same named-path pattern as flat-phase gate (a)). Reject within the 2-attempt budget (re-dispatch the offending sub-phase's Build without touching Red's tests or `exempt_authored` overlap paths — re-inject the `Deferred Phase Group: yes` flag on this re-dispatch, see the G4 flag-injection rule); escalate on second failure.
